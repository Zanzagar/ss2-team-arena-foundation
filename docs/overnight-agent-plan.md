...
- Run `git ls-files` on every path the brief names, before launch.
...
6. **"Every committed divergence report is tied to real evidence."** The digest
   ...
   family, and a fully forged armoured report is accepted by all 586 tests.
...
11. **"586 is the right number."** The suite grew by ratchet, one hardening pass
    at a time. Ask what fraction of it would still pass against a stub
